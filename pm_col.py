#! /usr/bin/env python
# Revision 1.0 by laurance.gao@ericsson.com 20170619
import time
import datetime
import sys, os, re
import tarfile
...
def make_files_list(target_dir, regx, delay):
    ...
    try:
        ...
        logging.info ("There are " + str(len(files)) + " files to be zipped")
        for filename in files:
            mod_time = os.path.getmtime(filename)
            logging.debug (curr_time - mod_time)
            if (delay > curr_time - mod_time >= 0):
                tar.add(filename)    
        tar.close() 
...
if __name__ == '__main__':
    ...
    make_files_list(work_dir, file_pattern, mtime_inmins*60)
